...
//-------------------------------------------------------------------------------------------------------------------
void ips200_showchar(uint16 x,uint16 y,const int8 dat,uint16 color)
{
	uint8 i,j;
	uint8 temp;
	...
	for(i=0; i<16; i++)
	{
		...
		for(j=0; j<8; j++)
		{
			if(temp&0x01)	ips200_wr_data16(color);
...
//-------------------------------------------------------------------------------------------------------------------
void ips200_showstr(uint16 x,uint16 y,const int8 dat[],uint16 color)
{         
	uint16 j;
	...
	while(dat[j] != '\0')
	{
		ips200_showchar(x+8*j,y*16,dat[j],color);
		j++;
	}
...
//-------------------------------------------------------------------------------------------------------------------
void ips200_showint8(uint16 x,uint16 y,int8 dat,uint16 color)
{
	uint8 a[3];
	uint8 i;
	if(dat<0)
	{
		ips200_showchar(x,y*16,'-',color);
		dat = -dat;
	}
	else	ips200_showchar(x,y*16,' ',color);
	...
	while(i<3)
	{
		ips200_showchar(x+(8*(i+1)),y*16,'0' + a[i],color);
		i++;
	}
}
...
//-------------------------------------------------------------------------------------------------------------------
void ips200_showuint8(uint16 x,uint16 y,uint8 dat,uint16 color)
{
	uint8 a[3];
	uint8 i;
	...
	while(i<3)
	{
		ips200_showchar(x+(8*i),y*16,'0' + a[i],color);
		i++;
	}
}
...
//-------------------------------------------------------------------------------------------------------------------
void ips200_showint16(uint16 x,uint16 y,int16 dat,uint16 color)
{
	uint8 a[5];
	uint8 i;
	if(dat<0)
...
//-------------------------------------------------------------------------------------------------------------------
void ips200_showuint16(uint16 x,uint16 y,uint16 dat,uint16 color)
{
	uint8 a[5];
	uint8 i;
...
//-------------------------------------------------------------------------------------------------------------------
void ips200_showint32(uint16 x,uint16 y,int dat,uint8 num,uint16 color)
{
    int8    buff[34];
    uint8   length;
    ...
    num++;
    if(0>dat)   length = sprintf(&buff[0],"%d",dat,color);//负数
    else
    {
        buff[0] = ' ';
        length = sprintf(&buff[1],"%d",dat,color);
        length++;
    }
    while(length < num)
    {
    ...
    ips200_showstr(x, y, buff,color);	//显示数字
}
...
//                              负数会显示一个 ‘-’号   正数显示一个空格
//-------------------------------------------------------------------------------------------------------------------
void ips200_showfloat(uint16 x,uint16 y,double dat,int8 num,int8 pointnum,uint16 color)
{
    uint8   length;
	int8    buff[34];
    ...
    if(0>dat)   length = sprintf( &buff[0],"%f",dat,color);//负数
    else
    {
